...
config-restore() {
  ...
  if ! command -v fzf >/dev/null 2>&1; then
    echo "❌ fzf required for interactive restore. Install with: sudo apt install fzf"
    return 1
  fi
...
emacs-restart() {
  ...
  if systemctl --user is-active emacs.service >/dev/null 2>&1; then
    ...
    if systemctl --user is-active emacs.service >/dev/null 2>&1; then
      ...
      echo "💡 Check status with: systemctl --user status emacs.service"
    fi
  else
    echo "⚠️ Systemd service not found or inactive"
...
emacs-restart-alternative() {
  ...
  case $choice in
    1)
      ...
      ;;
    2)
      echo "🔄 Restarting via emacsclient..."
      emacsclient -e '(kill-emacs)' 2>/dev/null
      sleep 2
      emacs --daemon
      sleep 3
      echo "✅ Daemon restart completed"
      ;;
    3)
      echo "🔧 Manual restart instructions:"
...
setup-literate-config() {
  ...
  case $choice in
    ...
    2)
      ...
      config-init-dotfiles
      echo ""
      echo "💡 Run 'make' to generate configs, then 'config-sync-to-dotfiles' to sync"
      ;;
    *)
  ...
  echo "💡 Use 'make' to tangle configurations"
