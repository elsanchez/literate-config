#!/usr/bin/env python3
"""
Textual Script Runner - Basic Working Version
Compatible with Textual 3.5.0
"""
                    ...
                    'scripts': [
                        {
                            ...
                            'help_text': 'Demo deployment script with various UI components',
                            'preview_mode': True
                        },
                        {
                            'name': 'backup-demo',
...
""" + "\n".join(f"  {cat}: {count}" for cat, count in categories.items())
    ...
    def action_reload_config(self):
        ...
        async def reload():
            ...
            log = self.query_one("#main-log", Log)
            log.write_line("✅ Configuration reloaded")
    ...
    def action_help(self):
        ...
        log.write_line("  Click on scripts to execute")
        log.write_line("  Use tree navigation on left")
        log.write_line("  Switch tabs for different views")

def main():
    parser = argparse.ArgumentParser(description="Textual Script Runner - Basic Version")
