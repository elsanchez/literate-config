#!/usr/bin/env python3
"""
Textual Script Runner - Working Version
Simple but functional TUI with all components
"""
...
try:
    ...
    # Make available outside try block
    _Binding = Binding
    _on = on
    ...
    class ScriptConfigScreen(Screen):
        ...
        @_on(Button.Pressed, "#cancel")
        def cancel_config(self):
            self.app.pop_screen()

        @_on(Button.Pressed, "#preview")
            ...
            for arg in self.script.args:
                ...
                try:
                    if arg.type == "checkbox":
                        ...
                        cmd.extend([f"--{arg.name}", str(slider.value)])
                    else:
    ...
    # Dummy classes and functions for when Textual is not available
    class ScriptConfigScreen:
        pass
...
class TextualScriptRunner(App):
    ...
    BINDINGS = [
        _Binding("ctrl+r", "reload_config", "Reload"),
        _Binding("ctrl+q", "quit", "Quit"),
        _Binding("f1", "help", "Help"),
    ]
    
    TITLE = "🚀 Textual Script Runner"
    SUB_TITLE = "Working TUI Interface"
    ...
    async def on_mount(self):
        ...
        log.write_line("🚀 Textual Script Runner - Working Version")
        log.write_line("========================================")
        log.write_line("✅ All UI components available:")
        log.write_line("   📝 Text Input, 🔘 Select, ☑️ Checkbox")
        log.write_line("   🎚️ Slider, 🔢 Number Input")
        log.write_line("")
                    ...
                    'scripts': [
                        {
                            ...
                            'description': 'Demo deployment script',
                            'category': 'Deployment',
                            'args': [
                                {
                                    'name': 'environment',
                            ...
                            'help_text': 'Demo deployment script showcasing all UI components',
                            'preview_mode': True
                        },
                        {
                            'name': 'backup-demo',
                            'path': './demo-scripts/backup.sh',
                            'description': 'Demo backup script',
                            'category': 'Database',
                            'args': [
                                {
                                    'name': 'compression',
                            ...
                            'tags': ['backup', 'demo'],
                            'help_text': 'Demo backup script with various configuration options',
                            'preview_mode': True
                        },
                        {
                            'name': 'simple-demo',
...
""" + "\n".join(f"  {cat}: {count}" for cat, count in categories.items())
    ...
    def action_help(self):
        ...
        log.write_line("   🎚️ Slider - Numeric ranges with visual feedback")
        log.write_line("   🔢 Number Input - Numeric values")
        log.write_line("")
        log.write_line("🎮 Try the demo scripts to see all components in action!")

def main():
    parser = argparse.ArgumentParser(description="Textual Script Runner - Working Version")
    ...
    if not HAS_TEXTUAL:
        ...
        print("💡 Try: source venv/bin/activate && uv pip install textual")
        sys.exit(1)
