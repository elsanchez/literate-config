...
(when (eq system-type 'darwin)
  (global-set-key (kbd "s-s") #'save-buffer)           ; cmd+s for save
  (global-set-key (kbd "s-S") #'write-file)            ; cmd+S for save as
  (global-set-key (kbd "s-x") #'execute-extended-command)) ; cmd+x for M-x (SPC :)
...
;; Jira/Confluence authentication functions (macOS only)
(when (eq system-type 'darwin)
  ...
  (defun my/get-confluence-credentials-from-keychain ()
    "Get Confluence credentials from macOS Keychain."
    (when (executable-find "security")
      (let ((url (shell-command-to-string
                  "security find-generic-password -a work -s confluence-url -w 2>/dev/null"))
            (token (shell-command-to-string
                    "security find-generic-password -a work -s confluence-token -w 2>/dev/null")))
        (when (and url token
                   (not (string-empty-p (string-trim url)))
                   (not (string-empty-p (string-trim token))))
          (list (string-trim url) (string-trim token)))))))
...
;; Jira/Confluence Integration (macOS only)
(when (eq system-type 'darwin)
  ;; Jira Integration
  ...
  (when-let ((confluence-creds (my/get-confluence-credentials-from-keychain)))
    (use-package! ox-confluence
      :after ox
      :config
      (setq ox-confluence-url (nth 0 confluence-creds)
            ox-confluence-space-key "TEAM"))
    
    (use-package! restclient
      :mode ("\\.http\\'" . restclient-mode)
      :config
      (defun my/confluence-rest-template ()
        "Insert REST client template for Confluence API."
        (interactive)
        (let ((url (nth 0 confluence-creds))
              (token (nth 1 confluence-creds)))
          (insert (format "# Confluence REST API Template
# Base URL: %s
# Authentication: Bearer token

### Get all spaces
GET %s/rest/api/space
Authorization: Bearer %s
Content-Type: application/json

### Get space content
GET %s/rest/api/space/TEAM/content
Authorization: Bearer %s
Content-Type: application/json

### Create page
POST %s/rest/api/content
Authorization: Bearer %s
Content-Type: application/json
...
"
                          url url token url token url token))))))
  
  ;; Jira/Confluence keybindings (only when credentials are available)
  (when (or (my/get-jira-credentials-from-keychain) 
            (my/get-confluence-credentials-from-keychain))
    (map! :leader
          (:prefix ("j" . "jira/confluence")
...
j          :desc "Update Jira issue" "u" #'org-jira-update-issue
           :desc "Browse Jira issue" "b" #'org-jira-browse-issue
           :desc "Export to Confluence" "e" #'ox-confluence-export-as-confluence
           :desc "REST client template" "r" #'my/confluence-rest-template
...
;; Jira/Confluence packages (macOS only)
(when (eq system-type 'darwin)
  (package! jiralib2)
  (package! org-jira)
