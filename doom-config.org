...
(defun elsanchez/doom-reload-config ()
  "Tangle config, doom sync, then restart the Emacs daemon and open a new GUI frame."
  (interactive)
  (let* ((doom-bin (or (executable-find "doom")
                       (expand-file-name "~/.config/emacs/bin/doom")))
         (cmd (when (and doom-bin (file-executable-p doom-bin))
                (format "make && %s sync" doom-bin))))
    (unless cmd
      (user-error "Cannot find Doom binary!"))
    ;; Kick off the compile and get its buffer
    (let ((comp-buf (let ((default-directory "~/org/literate-config/"))
                      (compile cmd))))
      ;; Once compile finishes, grab its process and attach a sentinel
            ...
            (set-process-sentinel
             ...
             (lambda (process event)
               (when (and (memq (process-status process) '(exit signal))
                 ...
                 ;; Successful build → ask and restart
                 (when (y-or-n-p "Build OK. Restart daemon & open frame? ")
               ...
               (unless (= (process-exit-status process) 0)
                 (message "⚠️ Build failed (%s)" event))))))))))

(defun elsanchez/doom-reload-config ()
  "Tangle config, doom sync, restart Emacs daemon y luego open a new GUI frame."
  (interactive)
  ;; Lanzar compile + sync
  (let* ((doom-bin (or (executable-find "doom")
                       (expand-file-name "~/.config/emacs/bin/doom")))
         (cmd   (and doom-bin (file-executable-p doom-bin)
                    (format "make && %s sync" doom-bin))))
    (unless cmd
      (user-error "Cannot find Doom binary!"))
    (let ((default-directory "~/org/literate-config/"))
      (compile cmd)))
  ;; Sentinel dentro del compile…
           ...
           (when (and (memq (process-status process) '(exit signal))
             ...
             (when (y-or-n-p "Build ok. Restart daemon & open frame? ")
               ...
               (start-process-shell-command
                ...
                "systemctl --user restart emacs.service")
               ;; Debug message
               (message "⏳ Esperando 3s para emacsclient…")
               ;; Launch client after delay
               (run-at-time
                "3 sec" nil
                (lambda ()
                  (message "🚀 Ejecutando emacsclient -c")
                  (start-process
                   "emacs-new-frame" "*emacs-new-frame-log*"
...
(use-package! emacs-claude-code
  :after vterm
  :config
  ;; Función para obtener API key de Secret Service
  ...
  ;; Configurar variable de entorno automáticamente
  (when-let ((api-key (my/get-anthropic-api-key-from-secret-service)))
    (setenv "ANTHROPIC_API_KEY" api-key)
    (message "✓ API key cargada desde Secret Service"))
  
  ;; Configuración de auto-respuestas
  (setq ecc-auto-response-responses 
        '((:y/n . "1")                    ; Responde "1" a Y/N
          (:y/y/n . "2")                  ; Responde "2" a Y/Y/N  
          (:waiting . "/user:auto")       ; Comando auto cuando espera
          (:initial-waiting . "/user/understand-guidelines"))) ; Comando inicial
  
  ;; Configuraciones adicionales
  (setq ecc-auto-refresh-enabled t        ; Auto-refresh de buffers
        ecc-auto-refresh-interval 2.0     ; Cada 2 segundos
        ecc-notification-enabled t)       ; Notificaciones de escritorio
  
  ;; Keybindings organizados bajo SPC c c
  (map! :leader
        (:prefix ("c c" . "claude-code")
         :desc "List Claude buffers"    "l" #'ecc-list-buffers
         :desc "Toggle auto-response"   "a" #'ecc-auto-toggle
         :desc "Switch host/machine"    "h" #'ecc-switch-host
         :desc "Toggle notifications"   "n" #'ecc-notification-toggle
         :desc "Toggle debug mode"      "d" #'ecc-debug-toggle
         :desc "Refresh buffer list"    "r" #'ecc-refresh-list))
  
  ;; Hook opcional para mostrar estado al iniciar
  (add-hook 'emacs-startup-hook
            (lambda ()
              (when (my/get-anthropic-api-key-from-secret-service)
                (message "🤖 Claude Code configurado correctamente")))))
